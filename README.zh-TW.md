...
| 4 | `wrong_language` ── 最後一則訊息不是設定裡指定的語言 | 本機計算 |
...
形狀 4 從 Claude Code 的設定讀 `language`，順序是專案的 `.claude/settings.local.json`、專案的 `.claude/settings.json`，最後是 `CLAUDE_CONFIG_DIR` 底下的使用者 `settings.json`，再拿最後一則訊息的正文去比。先拿掉 code（反引號或波浪號圍起來的，任意長度都算）、inline code、URL、引用區塊和路徑，這些在任何語言裡都是英文。剩下的部分以漢字對英文字詞計數，總數至少 12 個時，漢字不到 40% 就判定為錯語言。這個下限讓「OK」與一句簡短的狀態回報不被判斷。提示會要求用設定的語言把同一則訊息重寫一次，code 與識別字維持原樣。

只檢查中文目標。`language` 設成其他語言或沒設，形狀 4 就什麼都不做。
...
5. **`STINGRAY_LANG=1`（本機語言檢查）：** 最後一則訊息不是設定的 `language` 時攔下收尾。跟形狀 3 一樣在本機算：不需 key、不連網，只開這個開關時什麼都不會離開你的電腦。它不會順帶打開形狀 3。
6. **`STINGRAY_SHAPE3_JUDGE=1`（形狀 3 語意比對）：** 額外允許形狀 3 的判斷層攔下收尾（當背景有工作在跑，但模型判定在跑的工作與承諾不符）。必須同時啟用 `STINGRAY_SHAPE3=1` 與 `STINGRAY=1` 才會生效：只設形狀 3 時，hook 在進入 Jev 請求路徑前就會退出，這道判斷跑不到。即使啟用形狀 3 攔阻，此項預設仍然關閉，因為該門檻尚未經過離線量測。
...
TypeSafe 的資料處理位於美國境內，也未聲明資料保留期限。它的 Master Customer Agreement 把責任上限壓得夠低，真的外洩了拿不到實質的財務救濟。條文請讀現行版本，不要讀這裡的轉述──條文會改，轉述不會跟著改。啟用網路傳輸前請詳閱 `payload-audit.txt`。`STINGRAY_SHADOW=1` 依舊會送出資料。預設未設定，或只開本機開關（`STINGRAY_SHAPE3=1`、`STINGRAY_LANG=1`，或兩者）時，完全不發出網路請求──它們在進入請求路徑前就退出。
...
- 形狀 3 的準確率無法於離線環境量測：`background_tasks` 欄位僅存在於 hook 執行當下，無法由 transcript 重建。測試套件僅驗證合成資料下的程式邏輯，真實輪次的表現需依據個人的 shadow 紀錄判讀。
- 資料遮蔽機制具備明確上限，無法清除工作任務核心脈絡，詳見前述章節。
- 形狀 4 的門檻建立在一邊倒的樣本上。zh-TW session 的 2,967 則真實訊息裡有 2,834 則長度足以判斷，zh-TW 回覆的漢字占比沒有一則低於 60%，但真正是英文的回覆只有七則。門檻的 zh-TW 那一側量得很紮實，另一側只有七個例子。下限設 8 可以攔到 13 則英文回覆，但會讓簡短的 zh-TW 狀態回報離門檻只剩 5 點，所以沒採用。
- 形狀 4 分不出簡體與繁體，也不檢查中文以外的語言。
- harness 會把它自己的英文通知以助理訊息的形式寫進 transcript，例如「You've hit your session limit …」「API Error: Connection lost mid-response …」。hooks 文件寫明，因 API 錯誤而結束的一輪會觸發 `StopFailure` 而非 `Stop`，照這樣它們不會到這個 hook。這是文件依據，沒有實測。如果真的送到了，形狀 4 會攔一次，重新進入的防護會擋掉第二次。
...
- **案例 7：** 形狀 3 正規表達式必須在未遮蔽的原始文字上執行。宣告字句與檔案路徑同處一行；遮蔽機制會將包含路徑的整行移除。（先前版本曾以 URL 測試，但 URL 採就地替換而保留整行，致使錯誤實作通過驗證）。
