...
| `STINGRAY_SHAPE3=1` | 對形狀 3 擋人。**可以單獨使用**：「什麼都沒在跑」那種情況不需要 key 也不需要網路，所以光設這一個就能啟用 hook，而不會打開那兩個有自己門檻要過的 Jev 判斷。`STINGRAY_SHADOW=1` 的優先序高於它。 |
...
| `background` | 每個背景工作與排程的 status 與 **description**，走與訊息同一條剝除管線。指令列一律不送。 |
