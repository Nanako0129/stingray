...
`STINGRAY_SHAPE3` 是免費的本機監看檢查，不需帳號、不需 key、不連網。`STINGRAY_LANG` 是語言檢查，由 Jev 判斷，所以需要 API key，並會送出遮蔽過的最後一則訊息。已取得 API key 的話，改從 `"STINGRAY_SHADOW": "1"` 開始：呼叫 Jev、寫入決策紀錄，但絕不攔下收尾。改完檔案後重新啟動 Claude Code。
...
形狀 1、形狀 2 與語言檢查會呼叫 TypeSafe System One（`jev-1.13.0`）。形狀 3 的**確定**情況──承諾盯著工作但背景無任何工作或排程──不需 key 亦不需連網。對應判斷則需要 key 與網路連線，因為該項目需交由模型判定背景工作與承諾是否相符。

1. 前往 <https://typesafe.ai> 取得 key。
2. 存入 `~/.config/typesafe/api_key`（`chmod 600`），或設定環境變數 `TYPESAFE_API_KEY`。建議使用檔案儲存，環境變數容易暴露給子行程。

**未設定 key 時，stingray 保持形狀 1、形狀 2 與語言檢查關閉。** 它會在每個 session 輸出一次 `(stingray: unavailable — no key; Jev checks skipped)`，同時維持形狀 3 本機檢查運作，不會無聲停擺。
...
TypeSafe 的資料處理位於美國境內，也未聲明資料保留期限。它的 Master Customer Agreement 把責任上限壓得夠低，真的外洩了拿不到實質的財務救濟。條文請讀現行版本，不要讀這裡的轉述──條文會改，轉述不會跟著改。啟用網路傳輸前請詳閱 `payload-audit.txt`。`STINGRAY_SHADOW=1` 依舊會送出資料。預設未設定，或只開 `STINGRAY_SHAPE3=1` 時，完全不發出網路請求──它在進入請求路徑前就退出。`STINGRAY_LANG=1` 會跟形狀 1、2 一樣送出遮蔽過的最後一則訊息。
...
只開 `STINGRAY_LANG=1` 時，每一輪只要有正文可判斷，現在也會多這一次往返，以前則完全沒有。只帶語言題的請求在 12 次呼叫中耗時 0.68–1.03 秒──這是直接用相同格式的請求打端點量的，不是在 hook 執行點量的。同時開了 `STINGRAY=1` 時，語言題會併進形狀 1、2 本來就會發的那次請求，不會多一次往返。
