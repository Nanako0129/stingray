...
TypeSafe 的資料處理位於美國境內，也未聲明資料保留期限。它的 Master Customer Agreement 把責任上限訂為「前十二個月付款金額」與 50 美元**取高者**——所以 50 美元是下限，只有在你一毛都沒付的時候，它才等於整個上限。啟用網路傳輸前請詳閱 `payload-audit.txt`。`STINGRAY_SHADOW=1` 依舊會送出資料。完全不發出網路請求的只有兩種狀態：預設未設定，以及單獨設定 `STINGRAY_SHAPE3=1`——後者在進入請求路徑前就退出。
