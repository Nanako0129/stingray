...
形狀 4 從 Claude Code 的設定讀 `language`，順序是專案的 `.claude/settings.local.json`、專案的 `.claude/settings.json`，最後是 `CLAUDE_CONFIG_DIR` 底下的使用者 `settings.json`，再拿最後一則訊息的正文去比。先拿掉 code、inline code、URL、引用區塊和路徑，這些在任何語言裡都是英文。剩下的部分以漢字對英文字詞計數，總數至少 20 個時，漢字不到 40% 就判定為錯語言。提示會要求用設定的語言把同一則訊息重寫一次，code 與識別字維持原樣。
...
- 形狀 4 的門檻建立在一邊倒的樣本上。zh-TW session 的 2,967 則真實訊息裡，zh-TW 回覆的漢字占比全部在 63% 以上，40% 到 59% 之間一則都沒有，但真正是英文的回覆只有三則。門檻的 zh-TW 那一側量得很紮實，另一側只有三個例子。
- 形狀 4 分不出簡體與繁體，也不檢查中文以外的語言。
- harness 會不會把它自己的英文通知（「You've hit your monthly spend limit …」）當成助理的最後一則訊息交給 Stop hook，目前沒有量過。如果會，形狀 4 會攔一次，重新進入的防護會擋掉第二次。
