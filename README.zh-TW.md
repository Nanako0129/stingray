...
形狀 3 不會送到任何模型。它的兩半都是確定值：一個跑在助理自己話語上的宣告 regex，以及 hook 本來就會拿到的 `background_tasks` 欄位。它取代的那道 Jev 題目在同一份語料上只有 20.0% 精確率、66.7% 誤報率，所以改成算術不是妥協。它也不需要 key、不需要網路，因此 `STINGRAY_SHAPE3=1` 單獨使用就是最便宜的有用設定。
...
| `background` | 背景工作的 **status**——不含 description，不含指令列 |

你的 prompt 永遠不會被送出。工具參數、檔案內容、diff 也不會。
...
| 形狀 3（`STINGRAY_SHAPE3=1`） | 累積 ≥ 20 筆 shadow 紀錄、人工判讀精確率 ≥ 70% |
