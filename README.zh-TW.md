...
**形狀 3 的承諾判斷在 59 則標好答案的句子上量過**──`tests/watch-fixture.tsv` 的 44 則與 15 則合成句子──`tests/watch-fixture.sh` 會透過出貨的 hook 即時重跑：τ = 0.5 時，有評分的 43 則裡 42 則一致。第 44 則含有檔名，遮蔽後整則消失，從未被問到（見已知限制）。**它的對應判斷從未量過。** 門檻沿用其他題目的數值，未經獨立驗證。
