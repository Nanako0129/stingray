...
TypeSafe 的資料處理位於美國境內，也未聲明資料保留期限。它的 Master Customer Agreement 把責任上限壓得夠低，真的外洩了拿不到實質的財務救濟。條文請讀現行版本，不要讀這裡的轉述──條文會改，轉述不會跟著改。啟用網路傳輸前請詳閱 `payload-audit.txt`。`STINGRAY_SHADOW=1` 依舊會送出資料。完全不發出網路請求的只有兩種狀態：預設未設定，以及單獨設定 `STINGRAY_SHAPE3=1`──後者在進入請求路徑前就退出。
