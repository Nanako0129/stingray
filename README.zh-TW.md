...
魟魚貼在沙地上不動，沒踩到就不會有動靜。這個 hook 原理相同：正常對話保持安靜，只有在該繼續做卻提早收尾時介入。形狀 1 與形狀 2 屬於語意判斷，交給 [Jev](https://typesafe.ai) 裁決。形狀 3 則優先由本機算術清點；背景真有工作在跑時，才送交模型比對承諾與排程是否相符。
...
| 3 | `unwatched` ── 承諾盯著外部進度（CI、建置、PR 審查） | 本機算術或 Jev（詳見下文） |

形狀 3 明確切分為兩層，兩者原理不同：

- **確定層：** 助理在回覆中宣告要盯著外部進度，而執行中的背景工作數與 `session_crons` 數**總和等於 0**。此時由本機算術直接判定承諾落空，不需 API key，亦不發送網路請求。單獨設定 `STINGRAY_SHAPE3=1` 即可執行此層檢查。
- **判斷層：** 背景工作與排程數**總和大於 0**。此時單純計數無法確認執行中的工作是否符合承諾。例如背景正在執行建置，而承諾追蹤的是 PR 審查，非零的背景數會漏抓未履行的承諾。比對正在跑的工作與當初承諾是否一致屬於語意判斷，會送交 Jev 的 `watch_mismatch` 題目。未設定 API key 時跳過此判斷，維持原本的收尾行為。

先前版本曾宣稱形狀 3「兩半都是確定值」，該說法並不屬實。`background_tasks` 確實是 harness 傳來的確定欄位，但「是否宣告要盯著外部進度」是由正規表達式近似的語言判斷。
...
形狀 1 與形狀 2 會呼叫 TypeSafe System One（`jev-1.13.0`）。形狀 3 的**確定**情況──承諾盯著工作但背景無任何工作或排程──不需 key 亦不需連網。對應判斷則需要 key 與網路連線，因為該項目需交由模型判定背景工作與承諾是否相符。

1. 前往 <https://typesafe.ai> 取得 key。
2. 存入 `~/.config/typesafe/api_key`（`chmod 600`），或設定環境變數 `TYPESAFE_API_KEY`。建議使用檔案儲存，環境變數容易暴露給子行程。

**未設定 key 時，stingray 保持形狀 1 與形狀 2 關閉。** 它會在每個 session 輸出一次 `(stingray: unavailable — no key; shapes 1/2 skipped)`，同時維持形狀 3 本機檢查運作，不會無聲停擺。
...
2. **`STINGRAY_SHAPE3=1`（本機算術檢查）：** 單獨啟用形狀 3 的確定情況。當助理宣告盯著背景工作，而本機背景工作與排程數為 0 時，攔下那一輪的收尾。不需要 API key，不發送網路請求。
...
5. **`STINGRAY_SHAPE3_JUDGE=1`（形狀 3 語意比對）：** 額外允許形狀 3 的判斷層攔下收尾（當背景有工作在跑，但模型判定在跑的工作與承諾不符）。必須同時啟用 `STINGRAY_SHAPE3=1` 與 `STINGRAY=1` 才會生效：只設形狀 3 時，hook 在進入 Jev 請求路徑前就會退出，這道判斷跑不到。即使啟用形狀 3 攔阻，此項預設仍然關閉，因為該門檻尚未經過離線量測。

| 變數 | 效果 |
|---|---|
| *（未設定）* | **預設。** hook 立即以 exit 0 退出。不執行檢查、不發送請求、不建立狀態檔。 |
| `STINGRAY_SHADOW=1` | 呼叫 Jev 評估並記錄決策，**絕不攔下收尾**。優先序高於 `STINGRAY=1` 與 `STINGRAY_SHAPE3=1`。有 key 時建議由此開始。 |
| `STINGRAY=1` | 依 Jev 判讀結果，在形狀 1 與形狀 2 觸發時攔下收尾。 |
| `STINGRAY_SHAPE3=1` | 針對形狀 3 的**確定**情況攔下收尾（承諾盯著工作，但背景工作與排程數總和為 0）。不需 key 亦不需連網。優先序低於 `STINGRAY_SHADOW=1`。 |
...
3. 離線實驗採用的是遮蔽工具的**測試副本**（使用寫死的專案清單），出貨版本則改為動態推導。81.8% 係於近似出貨程式碼之環境測得，非實際出貨程式碼之精準量測。

**形狀 3 的對應關係判讀未曾執行離線量測。** 其判定門檻係沿用另兩項題目的數值，未經獨立驗證。

任一判斷機制在獲准攔下收尾前，必須各自通過兩道獨立檢驗標準：

| 評估項目 | 獲准攔下收尾之門檻 |
|---|---|
| 形狀 1 與 2（`STINGRAY=1`） | 累積 ≥ 40 筆 shadow 紀錄、人工判讀精確率 ≥ 70%、每 100 次收尾誤推 ≤ 3 次、τ 值落於分數叢集間之空隙並附推導過程 |
| 形狀 3 確定情況（`STINGRAY_SHAPE3=1`） | 無門檻──本機算術判定，不需校準數值 |
| 形狀 3 對應判斷（`STINGRAY_SHAPE3_JUDGE=1`） | 累積 ≥ 20 筆 shadow 紀錄、人工判讀精確率 ≥ 70% |

判定紀錄寫入 `$STINGRAY_STATE_DIR/decisions.jsonl`，一行一筆，附帶 `qset_hash`──即 `questions.json` 之 SHA-256 雜湊，**非請求內容之雜湊**。修改判斷標準字句會改變模型評分分佈，導致既有門檻失效；若使用每輪變動的請求雜湊，將無法追蹤題本版本漂移。

## 延遲

在 shadow 模式下於 hook 執行點量測台灣至 `api.typesafe.ai` 之連線耗時，共執行三輪獨立測試，每輪 20 次請求：

| | p50 | p95 | 預算 |
|---|---|---|---|
| shadow 模式 | 0.742–0.760 秒 | 0.818–0.888 秒 | 1.0 秒 |

測試結果刻意標記為數值區間，因為跨輪次之連線延遲無法精準重現，標示單一數值會造成誤導。

1.0 秒之延遲預算同樣適用於 shadow 模式：shadow 模式承擔相同的網路往返時間。若 p95 超出預算，應停用 hook 或調低 `STINGRAY_TIMEOUT`。退回 shadow 模式無法消除延遲負擔。

上述預算僅適用於端點連線正常之狀況。當端點無回應時，連線將等待至 `STINGRAY_TIMEOUT`：**在預設 6 秒設定下實測為 6.15 秒**。若此耗時在斷線時過長，請調低 timeout。

## 迴圈防護

具備兩道獨立防禦機制，避免單一布林值失效而導致對話無窮迴圈：

1. harness 提供之 `stop_hook_active` 旗標──重新進入 hook 時為 `true`，防止對同一收尾事件重複介入。
2. 每個 session 獨立計算之攔阻收尾次數上限（`STINGRAY_MAX_BLOCKS`，預設 3 次），不依賴 harness 旗標狀態。

## 已知限制

- `questions.json` 內之判斷標準以繁體中文撰寫，此為測得 81.8% 這個數字的語料基礎。英文標準未經實測驗證，任意替換將導致該準確率指標失效。在英文環境使用需重寫題本並重新校準。
- 形狀 3 之準確率無法於離線環境量測：`background_tasks` 欄位僅存在於 hook 執行當下，無法由 transcript 重建。測試套件僅驗證合成資料下的程式邏輯，真實輪次之表現需依據個人的 shadow 紀錄判讀。
- 資料遮蔽機制具備明確上限，無法清除工作任務核心脈絡，詳見前述章節。
- `network.sh --live` 於某次測試曾出現 8/9 之結果；後續重複執行五次皆無法重現該錯誤，未能查明特定失敗項目。該紀錄已載於測試檔案註解。
...
hook 實際接收之 JSON 欄位：`session_id`、`prompt_id`、`transcript_path`、`cwd`、`permission_mode`、`hook_event_name`、`stop_hook_active`、`last_assistant_message`、`background_tasks` 以及 `session_crons`。
...
│   ├── stingray.sh              # 完整邏輯：三種形狀、資料遮蔽、fail-open 路徑、提示字串
│   └── turn-tools.jq            # 從 transcript 切出單一對話輪次
├── questions.json               # 三道 Jev 判斷標準──分類器契約，釘死 jev-1.13.0
└── tests/
...
- **案例 7：** 形狀 3 正規表達式必須在未遮蔽之原始文字上執行。宣告字句與檔案路徑同處一行；遮蔽機制會將包含路徑之整行移除。（先前版本曾以 URL 測試，但 URL 採就地替換而保留整行，致使錯誤實作通過驗證）。
- **案例 9：** 輸入缺少 `background_tasks` 欄位時，不得誤判為「無任何工作在執行」，否則會導致形狀 3 在正規表達式命中時一律攔下收尾。

## 支持

stingray 免費開源且不需註冊專案帳號。使用上唯一產生的費用為使用者自身呼叫 TypeSafe API 之開銷（每百萬輸入 token 美金 0.042 元；單輪對話傳輸量遠低於一千 token）。歡迎透過 Patreon 支持維護者。
