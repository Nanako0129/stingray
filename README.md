...
**Shape 3's promise judgement is measured on the 50 labelled lines of `tests/watch-fixture.tsv`.** Sent to Jev directly in one pass, promises scored 0.53 and above and everything else 0.22 and below. `tests/watch-fixture.sh` re-runs them live through the shipped hook: at τ = 0.5 it agrees on all 49 it scores, with "keep an eye on the review" at the edge (see Known limits). The 50th names a file, so redaction removes it and it is never asked. The wording until 0.3.0 was measured on 59 lines (these 44 of the time and 15 synthetic, no longer kept); 0.3.1 stopped it counting "do this and tell me, then I'll read the log" as a promise, after such a reply blocked a real turn (replayed, it scores 0.53–0.58 under the old wording). On the same 50 lines the old wording gets 3 wrong, that reply among them, and scores a non-promise as high as 0.82. **Its correspondence judgement has never been evaluated.** Its threshold is borrowed from the other questions without independent validation.
...
| Shape 3, promise (`STINGRAY_SHAPE3=1`) | None set. Measured on 50 labelled lines, and blocks from the first turn — see known limits |
